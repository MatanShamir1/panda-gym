...
from panda_gym.envs.tasks.reach import Reach, ReachPredefinedPositions
...
class PandaReachSimpleEnv(RobotTaskSimpleStateEnv):
    """Push task wih Panda robot.
    ...
    def __init__(
        ...
        **kwargs
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = ReachPredefinedPositions(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, goal=kwargs.get("goal"))
        super().__init__(
            robot,
            task,
            render_width=render_width,
