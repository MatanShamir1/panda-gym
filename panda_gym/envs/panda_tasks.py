...
from panda_gym.envs.core import RobotTaskEnv, RobotTaskSimpleStateEnv
from panda_gym.envs.robots.panda import Panda
from panda_gym.envs.tasks.flip import Flip
from panda_gym.envs.tasks.pick_and_place import PickAndPlace
from panda_gym.envs.tasks.push import Push, PushPredefinedPositions
from panda_gym.envs.tasks.reach import Reach
from panda_gym.envs.tasks.slide import Slide
from panda_gym.envs.tasks.stack import Stack, StackPredefinedPositions
...
class PandaStackSimpleEnv(RobotTaskSimpleStateEnv):
    """Stack task wih Panda robot.
    ...
    def __init__(
        ...
        **kwargs
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = StackPredefinedPositions(sim, reward_type=reward_type, goal1=kwargs.get("goal1"), goal2=kwargs.get("goal2"), obj1=kwargs.get("obj1"), obj2=kwargs.get("obj2"))
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaPushSimpleEnv(RobotTaskSimpleStateEnv):
    """Push task wih Panda robot.
    ...
    def __init__(
        ...
        **kwargs
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = PushPredefinedPositions(sim, reward_type=reward_type, goal=kwargs.get("goal"), obj=kwargs.get("obj"))
        super().__init__(
            robot,
            task,
            render_width=render_width,
